...
export default function LoginPage() {   
  ...
  const handleLogin = async (e) => {     
    ...
    if ((cleanInputUsername === 'ADMIN' || cleanInputUsername === 'ADMİN') && password === 'admin1233') {       
      ...
      const { data: users, error } = await supabase
        .from('users')
        .select('*')
        .eq('username', cleanInputUsername)
  ...
  const handleVerifyAccount = async (e) => {     
    ...
    if (!error && matchedUsers && matchedUsers.length > 0) {
      matchedUser = matchedUsers.find(u => 
        (u.birthDate ? u.birthDate === verifyBirthDate : true) &&
        (u.phone ? u.phone.trim() === verifyPhone.trim() : true) &&
        (u.securityQuestion ? u.securityQuestion === verifySecurityQuestion : true) &&
        (u.securityAnswer ? u.securityAnswer.trim().toLocaleUpperCase('tr-TR') === cleanAns : true)
      );
    }
  ...
  const handleSaveNewPassword = async (e) => {     
    ...
    await supabase.from('users').update({ password: newPassword }).eq('id', targetUserObj.id);
  ...
  const handleSendSupportRequest = async (e) => {     
    ...
    const newReq = {       
      ...
      category: 'Şifre Sıfırlama Talebi',
      urgency: 'Çok Acil',
      description: `Şifre sıfırlama doğrulaması 3 kez hatalı girildi. Telefon: ${verifyPhone || 'Yok'}, Doğum Tarihi: ${verifyBirthDate || 'Yok'}`,
      date: new Date().toLocaleString('tr-TR'),       
      isRead: false     
    };     
    
    await supabase.from('system_issues').insert([newReq]);
